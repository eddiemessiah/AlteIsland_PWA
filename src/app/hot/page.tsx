...
export default function HotLeaderboard() {
  ...
  return (
    <div className="min-h-screen bg-[#0f0e10] text-white font-['Manrope'] pb-32 overflow-hidden">
      ...
      <main className="pt-24 px-6 max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12">
        
        {/* Left Column: Vetting Now */}
        <div className="lg:col-span-5 space-y-8 stagger-slide">
          <div className="glass-panel rounded-3xl p-8 glass-grain border border-white/5 shadow-2xl sticky top-28 voting-card">
            <div className="flex items-center justify-between mb-8">
              <h2 className="font-['Plus_Jakarta_Sans'] font-bold text-2xl text-[#ff89ab]">Vetting Now</h2>
            ...
            <div className="relative aspect-square rounded-2xl overflow-hidden mb-8 group">
              ...
              <div className="absolute bottom-6 left-6 right-6">
                <h3 className="font-['Plus_Jakarta_Sans'] text-3xl font-bold mb-1">Indigo Sky Lounge</h3>
                <p className="text-[#00e3fd] font-medium text-sm">Lagos, Nigeria</p>
              </div>
            </div>

            <div className="space-y-6">
              <p className="text-[#aeaaad] font-medium leading-relaxed italic">
                "The percussion layers are unmatched. Best acoustics in the city."
              </p>
              <div className="flex gap-4">
                <button 
                  onClick={() => handleVote("HOT")}
                  className="flex-1 bg-gradient-to-r from-[#ff89ab] to-[#e30071] text-white font-bold py-4 rounded-full flex items-center justify-center gap-2 active:scale-95 transition-transform shadow-[0_0_20px_rgba(255,137,171,0.3)]"
                >
                  <Flame className="w-5 h-5 fill-current" /> HOT
                </button>
                <button 
                  onClick={() => handleVote("NOT")}
                  className="flex-1 bg-[#272528]/40 border border-white/10 text-white font-bold py-4 rounded-full flex items-center justify-center gap-2 active:scale-95 transition-transform hover:bg-[#272528]"
                >
                  <Snowflake className="w-5 h-5" /> NOT
                </button>
              </div>
            </div>
            ...
            <div className="mt-8 pt-8 border-t border-white/5">
              <div className="flex items-center justify-between mb-4">
                <span className="text-sm font-['Manrope'] uppercase tracking-widest text-[#aeaaad]">Satisfaction</span>
                <span className="text-[#00e3fd] font-bold text-lg">{satisfaction}%</span>
              </div>
              <div className="h-4 w-full bg-[#141315] rounded-full overflow-hidden relative">
                <div className="h-full bg-gradient-to-r from-[#00e3fd] to-[#ff89ab] rounded-full relative liquid-melt shadow-[0_0_15px_rgba(0,227,253,0.5)]">
                  <div className="absolute right-0 top-0 h-8 w-2 bg-white/50 blur-sm rounded-full mix-blend-overlay"></div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="flex items-center justify-between mb-6 px-4 stagger-slide">
            <h2 className="font-['Plus_Jakarta_Sans'] font-extrabold text-3xl tracking-tight">
          ...
          <div className="space-y-3">
            {leaderboard.map((item, index) => (
              <div key={index} className="group glass-panel rounded-2xl p-4 flex items-center gap-6 hover:bg-[#272528]/80 transition-all duration-300 stagger-slide cursor-pointer border border-transparent hover:border-white/10">
                <div className="w-12 h-12 flex items-center justify-center font-['Plus_Jakarta_Sans'] font-black text-2xl text-[#00e3fd] opacity-40 group-hover:opacity-100 transition-opacity">
                  {item.rank}
                </div>
                ...
                <div className="flex-1">
                  <h4 className="font-['Plus_Jakarta_Sans'] font-bold text-xl mb-1">{item.name}</h4>
                  <p className="text-sm text-[#aeaaad] font-medium">{item.city}</p>
                </div>
